...
| Builder | `GET /wp-json/imperal/v1/builder`, `POST .../builder/field`, `GET .../builder/status`, `GET .../builder/scan`, `POST .../builder/heading` | Elementor and Bricks page-builder element trees, with guarded single-field point edits (never a whole-tree replace) and a guarded append-one-semantic-heading helper for Bricks zones. `GET .../builder/status` also reports `detected_builders`: a site-wide DETECTION-ONLY badge list for 49 other popular WordPress builders/block-libraries (WPBakery, Divi, Beaver Builder, Oxygen, Kadence Blocks, GenerateBlocks, Spectra, and so on) — active/inactive only, no read/write support for their content |
...
| Multisite | `GET /wp-json/imperal/v1/network/sites`, `POST .../network/sites`, `GET .../network/plugins` | Narrow network inventory and explicit subsite creation through WordPress core `get_sites()`, `get_plugins()`, and `wpmu_create_blog()`. Every route rejects non-Multisite installations and requires a network super-administrator. |
| Maintenance | `POST /wp-json/imperal/v1/maintenance/update-imperal-bridge`, `.../maintenance/update-plugin`, `.../maintenance/update-core`, `.../maintenance/run-due-cron`, `.../maintenance/install-plugin`, `.../maintenance/purge-cache`, `GET .../maintenance/list-plugins` | Update Imperal Bridge itself only from Imperal's fixed release ZIP, verified against the official SHA-256 manifest; or update one other plugin/WordPress core via the exact same `Plugin_Upgrader`/`Core_Upgrader` + `Automatic_Upgrader_Skin` classes wp-admin's own "Update Now" button and WordPress's background auto-updates use. The Bridge updater accepts no URL, package, or target from callers. This unlocks automatic future Bridge releases after version 2.17.0 is manually installed once. |
...
| User Sessions | `GET/DELETE /wp-json/imperal/v1/users/{id}/sessions` | Lists non-secret metadata for one user's native `WP_Session_Tokens` sessions (login, expiry, IP, user-agent) or ends all of that user's login sessions through `WP_Session_Tokens::destroy_all()`. Both routes require WordPress's `edit_users` capability; neither returns or accepts session tokens. |
