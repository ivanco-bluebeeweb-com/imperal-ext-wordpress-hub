...
| Maintenance | `POST /wp-json/imperal/v1/maintenance/update-plugin`, `.../maintenance/update-core`, `.../maintenance/run-due-cron` | Update one plugin or WordPress core via the exact same `Plugin_Upgrader`/`Core_Upgrader` + `Automatic_Upgrader_Skin` classes wp-admin's own "Update Now" button and WordPress's background auto-updates use, and force-run every past-due cron hook — all plain core upgrade/cron APIs that used to require SSH + WP-CLI's `wp plugin update`/`wp core update`/`wp cron event run --due-now` |
